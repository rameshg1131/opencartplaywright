...
```
Error: locator.click: Error: strict mode violation: getByAltText('Naukri Logo') resolved to 2 elements:
...
> 12 |     await page.getByAltText("Naukri Logo").click()
     |                                            ^ Error: locator.click: Error: strict mode violation: getByAltText('Naukri Logo') resolved to 2 elements:
  13 |     await page.waitForTimeout(2000)
  14 | })
  15 | 
  16 | //import { test, expect, chromium } from '@playwright/test';
  17 | 
  18 | /* test('Testing 1234', async () => {
  19 |   const browser = await chromium.launch({ headless: false });
  20 |   const context = await browser.newContext({ viewport: { width: 1250, height: 1200 } });
  21 |   const page = await context.newPage();
  22 | 
  23 |   await page.goto('https://www.facebook.com');
  24 |   await expect(page).toHaveTitle(/Facebook/);
  25 | 
